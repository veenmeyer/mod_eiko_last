...
while($i < $count_fields)
   { 
		...
		if (trim($fields[$i])=="Datum_Uhrzeit_Einsatzart_Einsatzort_Weiterlesen"):?><tr class="eiko_last_datum_uhrzeit_einsatzart_einsatzort_weiterlesen_tr"><td class="eiko_last_datum_uhrzeit_einsatzart_einsatzort_weiterlesen_td"><span class="eiko_last_datum_uhrzeit_einsatzart_einsatzort_weiterlesen_span"><?php echo date('d.m.Y ', $curTime).' um '.date('H:i', $curTime).' Uhr <b>'.$reports[$a]->einsatzart.'</b> '.$reports[$a]->address;?><a class="eiko_last_weiterlesen_link" href="<?php echo JRoute::_('index.php?option=com_einsatzkomponente&Itemid='.$mymenuitem.'&view=einsatzbericht&id=' . (int)$reports[$a]->id); ?>"><span class="eiko_last_weiterlesen_span"><?php echo $params->get('readon');?></span></a></span></td></tr><?php endif;
